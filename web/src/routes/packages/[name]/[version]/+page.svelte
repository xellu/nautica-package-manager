<script lang="ts">
    ...
    let selectedVersion = $derived(data.selectedVersion)
    let copied: boolean = $state(false);
</script>

<Embed
    title = {`${p.displayName} v${selectedVersion} | Nautica PR`}
...
<Page className="flex mt-16 gap-16 items-start max-md:flex-col">
    ...
    <div class="card preset-outlined-surface-100-900 p-3 md:w-72 w-full">
        ...
        <div class="flex w-full mt-3">
            <span class="card preset-filled-surface-100-900 p-1 px-3 border border-surface-200-800/50 font-mono grow text-sm">napi install {p.name}=={selectedVersion}</span>
            <button class="btn {copied ? 'preset-filled-primary-500' : 'bg-surface-200-800/70'} w-16"
                onclick={() => {
                    navigator.clipboard.writeText(`napi install ${p.name}==${selectedVersion}`)
                    copied = true;
    ...
    <!-- readme -->
    <div class="pb-16 grow max-w-screen overflow-y-scroll -mx-3 px-3">
